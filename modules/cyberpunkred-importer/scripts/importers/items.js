...
const OPTIONAL_DATABASES = ["cyberchairs"];
const databases = {};
...
export async function loadItemDatabases() {
    await Promise.all(Object.entries(DATABASE_IDS).map(async ([databaseId, databaseName]) => {
        ...
        } else if (OPTIONAL_DATABASES.includes(databaseId)) {
            console.warn(`CPR Importer: optional compendium ${databaseName} not found, skipping.`);
        } else {
...
async function importItemData(data, actor, databaseName, getItemName) {
    return await Promise.all(data[databaseName].map(async piece => {
        ...
        if (!db) {
            console.warn(`CPR Importer: database '${databaseName}' not loaded, skipping ${itemName}.`);
            return;
        }
        const data = db.getName(itemName);
        if (data) {
...
export async function importItems(data, actor) {
    ...
    // Bypass del diálogo modal de installCyberware() del sistema.
    // Se instala directo con installItems() preservando validación de slots/tipos.
    ...
    for (const item of otherCyberware) {
        const compatible = actor.itemTypes.cyberware.find(
            cw => cw.system.isInstalled && cw.system.isFoundational && cw.system.type === item.system.type
        );
        if (compatible) {
            await compatible.installItems([item]);
        } else {
            ui.notifications.warn(`No compatible foundational cyberware found for ${item.name}, installing on actor directly.`);
            await actor.installItems([item]);
        }
    }

    // Aplicar pérdida de humanidad estática (suma de todos los cyberware instalados)
    const allInstalled = actor.itemTypes.cyberware.filter(cw => cw.system.isInstalled);
    const totalStaticHL = allInstalled.reduce((sum, cw) => sum + cw.system.humanityLoss.static, 0);
    if (totalStaticHL > 0 && typeof actor.loseHumanityValue === 'function') {
        await actor.loseHumanityValue(allInstalled, 'static');
    }
